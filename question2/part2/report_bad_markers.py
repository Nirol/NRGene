...
def search_in_filtered_markers(marker_id, filteredmdarkers_df):
    for name in filteredmdarkers_df['Marker']:
        if name == marker_id:
            return True
    return False


def write_filterd_markers_fasta_file(markers_fasta):
    with open("output/part2/filterd_markers.fasta", 'w') as fp:
        fp.write("\n".join(markers_fasta))


def report_filtered_markers(filteredmdarkers_df):
    report_filtered_markers_csv(filteredmdarkers_df)
    fasta_markers = []
    ...
    for record in SeqIO.parse(file_path, "fasta"):
        ...
        if is_sequence_valid( sequence):
            marker_name, marker_id= extract_sequence_info(description)
            if search_in_filtered_markers(marker_id, filteredmdarkers_df ):
                fasta_description_line = ">" + marker_name
                fasta_markers.append(fasta_description_line)
                fasta_markers.append(str(sequence))
    write_filterd_markers_fasta_file(fasta_markers)
